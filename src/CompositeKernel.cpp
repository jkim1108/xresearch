...
CompositeKernel::CompositeKernel(Options opt) :
                    Kernel(opt), _useMult(opt.useMult), _lambda2(opt.lambda2)
                    {
                    }

double CompositeKernel::docKernel(const Graph* graph1, const Graph* graph2)
{
    double sum = 0;
...
double CompositeKernel::C(const Graph* graph1, const Graph* graph2, unsigned int i, unsigned int j, int l, preComputed& pre)
{
    ...
    else
    {
        ...
        newPre.value += _wordKernel(newPre.compVector1, newPre.compVector2) * pow(_lambda1, pre.length);
        if (pre.length > 0)
        {
            newPre.value +=  _wordKernel(pre.compVector1, newPre.compVector2) * pow(_lambda1, pre.length) * _lambda2;
            newPre.value +=  _wordKernel(pre.compVector2, newPre.compVector1) * pow(_lambda1, pre.length) * _lambda2;
        }
...
double CompositeKernel::docKernel(const depTree* dt1, const depTree* dt2)
{
    double sum = 0;
...
double CompositeKernel::C(const depTree* dt1, const depTree* dt2, int i, int j, int l, preComputed& pre)
{
    if ((l==0) or (dt1->nodeList[i]->height==0) or (dt2->nodeList[j]->height==0))
    {
        ...
        return _wordKernel(compVector1, compVector2) * pow(_lambda1, pre.length);
    }
    ...
    else
    {
        ...
        double sum = _wordKernel(newPre.compVector1, newPre.compVector2) * pow(_lambda1, pre.length);
        if (pre.length)
        {
            newPre.value +=  _wordKernel(pre.compVector1, newPre.compVector2) * pow(_lambda1, pre.length);
            newPre.value +=  _wordKernel(pre.compVector2, newPre.compVector1) * pow(_lambda1, pre.length);
        }
        newPre.length = pre.length + 1;
